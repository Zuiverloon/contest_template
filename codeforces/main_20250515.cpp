...
bool f(ll pre, vector<vector<ll>> &eg, ll n)
{
    vector<ll> degree(n + 1, 0);
    vector<set<ll>> nei(n + 1, set<ll>());
    queue<ll> q;
    ll tans = 0;
    for (ll i = 0; i < pre; i++)
    {
        degree[eg[i][1]]++;
        nei[eg[i][0]].insert(eg[i][1]);
    }
    for (ll i = 1; i < degree.size(); i++)
    {
        if (degree[i] == 0)
        {
            // cout << "push1 q " << i << "\n";
...
void solve(int cas)
{
    ll n, m;
    cin >> n >> m;
    vector<vector<ll>> eg(m, vector<ll>(2, 0));
    // vector<set<ll>> nei(n + 1, set<ll>());
    for (ll i = 0; i < m; i++)
    {
        cin >> eg[i][0] >> eg[i][1];
