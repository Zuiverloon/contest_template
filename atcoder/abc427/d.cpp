...
void f1(ll o, ll k, ll n, vector<set<ll>> &as, vector<set<ll>> &bs, map<ll, set<ll>> &pre, string &s);

void f2(ll o, ll k, ll n, vector<set<ll>> &as, vector<set<ll>> &bs, map<ll, set<ll>> &pre, string &s)
{
    if (o == k - 1)
    {
        for (ll i = 1; i <= n; i++)
...
void solve(int cas)
{
    ...
    vector<set<ll>> as(k, set<ll>());
    vector<set<ll>> bs(k, set<ll>());
    f1(0, k, n, as, bs, pre, s);
    if (isinset(as[0], 1LL))
    {
        cout << "Alice\n";
    }
